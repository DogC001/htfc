...
| button-primary-default-danger | `#E43340` | 规范色板中的风险按钮默认色 |
| button-primary-hover-danger | `#EA616B` | 规范色板中的风险按钮悬停色 |
| button-primary-disabled-danger | `#F6BBC0` | 规范色板中的风险按钮禁用色 |
...
- 未读消息徽标和品牌 logo 可保留红色；图表折线、柱线等数据序列可按图表色板使用橙色或多色，但不得反向影响普通控件选中态。
...
- 普通选中、高亮、hover、active、筛选命中、当前页、当前 Tab、当前菜单、当前商品或当前合约不得使用红色/浅红背景。
- 未读消息徽标可使用红色小面积提示；品牌 logo 按品牌资产保留；图表数据序列可使用图表色板中的橙色或多色。
...
- 使用浅蓝到浅青背景。
- 可使用柔和径向光斑，但只能作为低对比背景过渡，不做独立可见的圆形装饰。
...
:root {
  ...
  --tj-shadow-panel: 0 0 30px rgba(145, 152, 167, 0.25);
